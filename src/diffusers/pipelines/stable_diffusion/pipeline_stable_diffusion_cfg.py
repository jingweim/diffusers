...
class StableDiffusionPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        if shared_token:
            ...
            token_prompt_embeds = self.text_encoder(
                text_input_ids.to(device),
                attention_mask=attention_mask,
            )
            token_prompt_embeds = token_prompt_embeds[0]

            token_prompt_embeds = token_prompt_embeds.to(dtype=self.text_encoder.dtype, device=device)

            bs_embed, seq_len, _ = token_prompt_embeds.shape
            # duplicate text embeddings for each generation per prompt, using mps friendly method
            token_prompt_embeds = token_prompt_embeds.repeat(1, num_images_per_prompt, 1)
            token_prompt_embeds = token_prompt_embeds.view(bs_embed * num_images_per_prompt, seq_len, -1)
