...
def generate_step(pipe, prompt, shared_token, output_dir, seed, num_images_per_prompt, num_inference_steps,
                  guidance_scale, guidance_scale_db):

    # set seed
    set_seed(seed)

    # generate images
    images = pipe(prompt, shared_token, num_inference_steps=num_inference_steps, guidance_scale=guidance_scale, \
                  guidance_scale_db=guidance_scale_db, num_images_per_prompt=num_images_per_prompt).images
    ...
    with open(os.path.join(args.output_dir, 'inference_prompts.txt'), 'r') as f:
        ...
        for i, line in enumerate(lines):
            if line.startswith('##'):
                ...
                for seed in seeds:
                    output_dir = os.path.join(args.output_dir, 'inference_gsd_1-01', checkpoint, run, 'seed_%02d' % seed)
                    # skip if already generated
                    if os.path.exists(output_dir):
                        print(checkpoint, run, 'seed_%02d' % seed, 'skipped')
                    else:
                        os.makedirs(output_dir)
                        generate_step(pipe, prompt, shared_token, output_dir, seed, num_images_per_prompt, num_inference_steps,
                                      guidance_scale, guidance_scale_db)
                        ...
                        with open(os.path.join(output_dir, 'args.txt'), 'w') as f:
                            ...
                            f.write(f'guidance_scale_db = {guidance_scale_db}\n')
